#!/usr/bin/env python3
"""
Generate HTML visualization for San Diego County with 4% Z scaling.
"""
...
# Load San Diego subset data
print("Loading San Diego subset...")
data = np.loadtxt('/tmp/sandiego_subset.csv', delimiter=',')
lons = data[:, 0]
lats = data[:, 1]
potentials = data[:, 2]
...
fig.update_layout(
    title=f"San Diego County - Natural Topological Sampling (Linear Z @ 4% + Log Color)",
    scene=dict(
...
# Save HTML
output_path = 'output/usa_natural/sandiego_area_4pct.html'
