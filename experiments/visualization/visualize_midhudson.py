#!/usr/bin/env python3
"""
Generate HTML visualization for Mid-Hudson Valley with 4% Z scaling.
"""
...
# Load Mid-Hudson subset data
print("Loading Mid-Hudson subset...")
data = np.loadtxt('/tmp/midhudson_subset.csv', delimiter=',')
lons = data[:, 0]
lats = data[:, 1]
potentials = data[:, 2]
...
fig.update_layout(
    title=f"Mid-Hudson Valley - Natural Topological Sampling (Linear Z @ 4% + Log Color)",
    scene=dict(
...
# Save HTML
output_path = 'output/usa_natural/midhudson_area_4pct.html'
