#!/usr/bin/env python3
"""
Step 2: Calculate 1/d³ potential at triangle centers.
...
Output: output/usa/triangle_centers_d3_potential.csv (~145k: lon, lat, potential)

Performance: ~72 seconds for USA

Algorithm: Vectorized NumPy calculation
  - Processes triangle centers in chunks of 1000
  - For each chunk, calculates distance to ALL census tracts
  - Computes potential = Σ(population / distance³)
...
def main():
    print("="*70)
    print("USA 1/d³ POTENTIAL CALCULATION")
    print("="*70)
    print()
    ...
    # Calculate potential at each triangle center
    log("")
    ...
    for chunk_idx in range(num_chunks):
        ...
        # Clamp minimum distance to avoid near-singularities
        distances = np.maximum(distances, 0.001)

        # Calculate contributions: population / distance³
        # Optional: cap individual contributions at 500k
    ...
    # Save results
    output_path = Path("output/usa/triangle_centers_d3_potential.csv")
    ...
    print()
    print("="*70)
    print("POTENTIAL CALCULATION COMPLETE")
    print("="*70)
