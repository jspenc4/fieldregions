...
# Calculate potential at centers using GRAVITY (1/d potential from 1/d^2 force)
log('\nCalculating GRAVITY potential at triangle centers (1/d from 1/d^2 force)...')
...
for chunk_idx in range(num_chunks):
    ...
    # Calculate contributions: pop / d (gravity potential, not d^3!)
    contributions = tract_pops[np.newaxis, :] / distances

    # Cap at reasonable value (higher cap for 1/d since values are smaller)
    contributions = np.minimum(contributions, 50000)
...
# Aspect ratio - use ~9% z_aspect like the western hemisphere print
...
# PRINTABLE z_aspect: ~9% like the Oct 6 western hemisphere print (not 1%)
z_aspect = 0.09
log(f'Using PRINTABLE z_aspect = {z_aspect:.3f} (~9% like western hemisphere print)')

fig.update_layout(
    title='USA - GRAVITY (1/d potential) - PRINTABLE (9% z-aspect)',
    scene=dict(
...
output_path = 'output/usa_triangle_centers_gravity_printable.html'
fig.write_html(output_path)
log(f'\n✓ Saved to {output_path}')
log(f'\nDONE! USA gravity potential (1/d) PRINTABLE visualization complete.')
log(f'This version uses z_aspect = 0.09 (~9%) matching your western hemisphere print!')
